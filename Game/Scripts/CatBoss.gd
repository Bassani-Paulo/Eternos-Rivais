...
const bullet = preload("res://Scenes/CatBullet.tscn")
onready var shootTimer = $ShootTimer
onready var changeAttackTimer = $ChangeAttackTimer
onready var gun = $Gun

const rotateSpeeds = [120, 40, 0]
var shooterTimers = [0.25, 0.5, 1]
const spawnPointCounts = [2, 6, 18]
const radius = 200
...
var hp = maxHP
var healthy = true
onready var player = get_parent().get_node("RatPlayer")
...
func _process(delta):
	...
	get_parent().get_node("TextureProgress").value = hp
	if hp <= 0:
		get_tree().change_scene("res://Comics/Slide12.tscn")
	if (hp <= maxHP/3) and healthy:
			healthy = false
			get_node("WantToMoveTimer").stop()
			print(screenBorders/2)
			global_position = screenBorders/2
			setUpAttack()
	if isDescending:
...
func setUpAttack() -> void:
	for i in gun.get_children():
			i.queue_free()
	if healthy:
		var step = PI/spawnPointCount
		for i in range(spawnPointCount/2):
